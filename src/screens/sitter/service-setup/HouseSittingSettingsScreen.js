...
export default function HouseSittingSettingsScreen({ navigation }) {
    ...
    // Relative mode ON: additional rates are auto-computed from the base rate
    // (and locked in the UI). Turning it OFF leaves the last values in place so
    // the sitter can edit them manually. Recompute when toggled on or base changes.
    React.useEffect(() => {
    ...
    const fetchSettings = async () => {
        try {
            setIsLoading(true);
            const response = await getServiceSetup('HOUSE_SITTING');
            ...
            console.error('Failed to fetch house sitting settings:', error);
            setIsExistingData(false);
        } finally {
            setIsLoading(false);
        }
    ...
    const handleSave = async () => {
        ...
        try {
            ...
            const response = await upsertServiceSetup('HOUSE_SITTING', settings, true);
            if (response.success) {
                navigation.navigate('ProfileSetup', { completedService: 'houseSitting' });
            } else {
                alert('Failed to save settings. Please try again.');
            }
        } catch (error) {
            console.error('Error saving house sitting settings:', error);
            alert('Failed to save settings. Please try again.');
        } finally {
            setIsSaving(false);
        }
        ...
        <ScreenWrapper noBottomTabs>
            <View style={styles.container}>
                ...
                <View style={styles.header}>
                    ...
                    <Text style={styles.headerTitle}>House Sitting</Text>
                ...
                >
                    ...
                    <View style={styles.titleContainer}>
                        <Text style={styles.pageTitle}>Manage House Sitting</Text>
                    </View>
                    ...
                    <View style={styles.section}>
                        ...
                        <View style={styles.fieldGroup}>
                            <Text style={styles.fieldLabel}>What days are you generally available to offer House Sitting?</Text>
                    ...
                    <View style={styles.section}>
                        ...
                        <View style={styles.fieldGroup}>
                            <Text style={styles.fieldLabel}>What can pet owners expect when House Sitting at your home?</Text>
                            <Dropdown
